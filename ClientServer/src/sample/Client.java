...
public class Client{
 ...
 //add text field for file
  ...
  static   int setupstage= 0;
    static int sizem;// cumulative size od stream
    static int tc=1;
    static int dt=1;
    static int loss=0;// number of loss packet
    static long currentt=0;
    static long start;
  ...
  public Client() {
	...
	playButton.setToolTipText("Cannot Play,no connection established!");
    ...
    statusPanel.setBounds(10,370,50,20);
    statPanel.setBounds(60,370,440,20);

    f.getContentPane().add(mainPanel, BorderLayout.CENTER);
    f.setSize(new Dimension(500,420));
    f.setVisible(true);
...
//Establish a TCP connection with the server to exchange RTSP messages
  ...
  class setupButtonListener implements ActionListener{
    public void actionPerformed(ActionEvent e){
      ...
      playButton.setToolTipText("Ready to Play,press to start!");
      // if (state == INIT) 
      //{
  ...
  class timerListener implements ActionListener {
	...
	//print speed/bandwidth requirement
	    print_speed(rtp_packet);
  ...
  private void send_RTSP_request(String request_type)
  {
    ...
    // print RTSP connection state
    private void print_status(int state)
    {
	switch (state)
	    {
	case 0:
    ...
    private void print_speed(RTPpacket rtp_packet)
{
  
 
    //cumulative size of data received	
    sizem=sizem+rtp_packet.getpayload_length();
    //dt: time interval between consecutive packet
	dt=tc-rtp_packet.gettimestamp();
	//tc: packet timestamp	
	tc=rtp_packet.gettimestamp();
    	// packet loss in % in the case of fixed interval streaming
   if ((currentt-(rtp_packet.getsequencenumber()-1)!=0)) // if timestamp of packet received is greater than expected
	    {
			loss++;
	    }

	countpkt.setText("L"+loss+" "+tc/1000+":"+(tc-(tc/1000)*1000)/100+" s  Transmission rate: "+(sizem/tc)+" KB/s - Burst rate: "+(rtp_packet.getpayload_length()/(-dt))+ "KB/s");
   
	print_status(state);
	currentt=rtp_packet.getsequencenumber();
